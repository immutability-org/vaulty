...
import (
	"fmt"
	"os"
	"github.com/immutability-org/vaulty/libstring"
...
func newDbForTest(t *testing.T) *sqlx.DB {
	...
	db, err := sqlx.Connect("postgres", fmt.Sprintf("postgres://%v@%v:%v/vaulty-test?sslmode=%v", pguser, pghost, pgport, pgsslmode))
	if err != nil {
